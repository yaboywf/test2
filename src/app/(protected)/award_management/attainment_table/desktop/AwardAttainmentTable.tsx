...
const AwardAttainmentTable = ({ awards, milestone, boys, toggleAttainment, attained, personalMastery }: AwardAttainmentTableProps) => {
	...
	const rowsPerPage = 8;
	const [rankSelected, setRankSelected] = useState<string[]>(["Recruit (REC)", "Private (PTE)", "Lance Corporal (LCP)", "Corporal (CPL)", "Sergeant (SGT)", "Staff Sergeant (SSG)", "Warrant Officer (WO)"]);
	const [levelSelected, setLevelSelected] = useState<string[]>(["Sec 1", "Sec 2", "Sec 3", "Sec 4", "Sec 5"]);
	...
	const filteredBoys = useMemo(() => {
		return boys.filter(boy => {
			const matchesSearch = search === "" || boy.name.toLowerCase().includes(search.toLowerCase());
			const matchesRank = rankSelected.length === 7 || rankSelected.some(r => r.includes(`(${boy.rank})`));
			const matchesLevel = levelSelected.length === 0 || levelSelected.includes(`Sec ${getSecLvl(boy)}`);
			return matchesSearch && matchesRank && matchesLevel;
		});
	}, [boys, search, rankSelected, levelSelected]);

	const paginatedBoys = useMemo(() => {
		const start = (page - 1) * rowsPerPage;
		const end = start + rowsPerPage;
		return filteredBoys.slice(start, end);
	}, [filteredBoys, page]);

	const totalPages = Math.ceil(filteredBoys.length / rowsPerPage) || 1;
	...
	return (
		<div className={styles.container}>
			<div className={styles.header}>
				<AwardFilter search={search} rankSelected={rankSelected} levelSelected={levelSelected} setRankSelected={setRankSelected} setLevelSelected={setLevelSelected} setSearch={setSearch} />

				<div className={styles.pagination_container}>
					<p>Showing {filteredBoys.length === 0 ? 0 : page * rowsPerPage - rowsPerPage + 1} to {Math.min(page * rowsPerPage, filteredBoys.length)} of {filteredBoys.length} boys</p>
					{totalPages > 1 && (
			...
			<div className={styles.attainment_table} ref={scrollRef} key={milestone}>
				<div
					...
					style={{
						gridTemplateColumns: `200px repeat(${displayAwards.length + fixedRequirements[milestone].length}, 150px)`
					}}
				>
					<div>
						<p>Boys</p>

						{fixedRequirements[milestone].map(requirement => <p key={requirement}>{requirement}</p>)}
						{displayAwards.map(award => (
							<p key={award.id}>
								{"displayName" in award ? (award as any).displayName : award.badge_name}
							</p>
						))}
					</div>

					{paginatedBoys.map(boy => (
						<div key={boy.email} className={boyMeetsMilestone(boy.email) ? styles.completed : ""}>
							<section className={styles.boy}>
								<p>{boy.rank} {boy.name}</p>
								<section className={styles.tags}>
									<span data-sec={`Sec ${getSecLvl(boy)}`}>Sec {getSecLvl(boy)}</span>
								</section>
							</section>
							...
							{fixedRequirements[milestone].map(requirement => {
								...
								return (
									<section key={`${boy.email}-${requirement}`}>
										<input
							...
							{displayAwards.map(award => (
								<section key={`${boy.email}-${award.id}`}>
