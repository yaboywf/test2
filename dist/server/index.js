...
function renderToReadableStream(data, options, extraOptions) {
  return renderToReadableStream$1(data, options, { onClientReference(metadata2) {
    __vite_rsc_assets_manifest__.clientReferenceDeps[metadata2.id] ?? {};
  } });
}
...
function buildGoogleFontsUrl(family, options) {
  ...
  const styles2 = options.style ? Array.isArray(options.style) ? options.style : [options.style] : [];
  if (weights.length > 0 || styles2.length > 0) {
    const hasItalic = styles2.includes("italic");
    if (weights.length > 0) {
      if (hasItalic) {
        const pairs = [];
